function Enable-NetAdapter {
    ...
    .DESCRIPTION
        Brings a network adapter up using 'ip link set <name> up'. Requires root/sudo.
    .PARAMETER Name
        Name of the adapter to enable. Accepts pipeline input from Get-NetAdapter.
    .PARAMETER InterfaceIndex
        Interface index of the adapter to enable.
    .EXAMPLE
        Enable-NetAdapter -Name eth0
    .EXAMPLE
        Get-NetAdapter -Name eth0 | Enable-NetAdapter
    #>
    ...
    process {
        ...
        # Resolve name from index if needed
        $_name = $Name
        ...
        if ($PSCmdlet.ShouldProcess($_name, 'Enable-NetAdapter (ip link set up)')) {
            $result = ip link set $_name up 2>&1
            if ($LASTEXITCODE -ne 0) {
                $ex = [System.InvalidOperationException]::new(
                    "Failed to enable adapter '$_name': $result")
                $er = [System.Management.Automation.ErrorRecord]::new(
                    $ex, 'NetAdapter.Linux.EnableFailed',
